...
import { ISOSummarySkeleton } from "../LoadingSkeleton";
...
// import exportISOFindingsToExcel from "./ExportISOFindingsToExcel";
...
const ISOSummary = ({
  ...
  const [ISORules, setISORules] = useState(null);
  ...
  const [isISOSampleReport, setIsISOSampleReport] = useState(false);
  ...
  const [hiddenISOFindings, setHiddenISOFindings] = useState(() => {
    const saved = localStorage.getItem("hiddenISORules");
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    localStorage.setItem("hiddenISORules", JSON.stringify(hiddenISOFindings));
  }, [hiddenISOFindings]);

  const handleHideISOFinding = (e, findingId) => {
    // console.log("first: ", e, findingId);
    e.stopPropagation();
    setHiddenISOFindings((prev) => [...prev, findingId]);
  ...
  const handleReportAccountChange = async (accounts, isSample = false) => {
    ...
    try {
      ...
      if (apiResponse?.status === "ok") {
        ...
        const ISOScannedDataAllRegions = report.results;
        ...
        setISORules(ISOScannedDataAllRegions);
        processDashboardData(ISOScannedDataAllRegions);
      } else {
        notifyError(apiResponse?.error_message || "Failed to get report");
        setSelectedReportAccount(previousValue);
        setIsISOSampleReport(false);
      }
    } catch (err) {
  ...
  const handleScanClick = async () => {
    ...
    try {
      // const webSocketURL = `${process.env.REACT_APP_WEBSOCKET_URL}/ISO-progress`;
      ...
      const sendBackendRequest = async () => {
        setLoading(true);
        try {
          const response = await fetch(`${backend_url}/api/iso-scan`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          if (result?.status === "ok") {
            ...
            setLoading(false);
            setProgress(0);
            notifyError(result?.error_message || "Error in ISO rules scan");
            console.log("Error in ISO rules scan", result);
  ...
  const getFilteredTableData = () => {
    ...
    return dashboardData?.tableData.filter((item) => {
      ...
      const hiddenMatch = !hiddenISOFindings.includes(item.id + item.region);
  ...
  const exportToJSON = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_ISO-findings.json`,
      json,
      "application/json"
    );
  ...
  const exportToCSV = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_ISO-findings.csv`,
      csv,
      "text/csv"
    );
    ...
    },
    {
      ...
      render: (_, record) => (
        <Popconfirm
          ...
          onConfirm={(e) => handleHideISOFinding(e, record.id + record.region)}
          okText="Yes"
          cancelText="No"
      ...
      <div className="p-6 pl-12">
        <div className="max-w-7xl mx-auto">
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="mt-6 flex items-center gap-4">
                  ...
                  >
                    {s3FetchLoading && isISOSampleReport ? (
                      <span>
          ...
          {loading ? null : s3FetchLoading ? (
            ...
            <div>
              {isISOSampleReport && <GetSampleReportNote />}
              {/* Top Row - Charts */}
