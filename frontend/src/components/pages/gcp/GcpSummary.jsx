    ...
    const parsedProjects = (selectedProjects || [])
      .map((proj) => {
        try {
          ...
          console.error("Invalid project JSON:", proj);
          return null;
        }
      })
      .filter(Boolean);
        ...
        <div>
          {results.length ? (
            <div>
              ...
              <div className="mb-6">
                <OverviewCard findings={results} cloud="gcp" />
              </div>
            ...
            <NoDataAvailableMessageComponent
              messages={[
                "No data available.",
                "Select projects, then click 'Run Scan' to scan your GCP projects.",
                "If no projects appear in the dropdown, please add them from the Home page.",
              ]}
            />
          )}
        </div>
