...
import EKSRoleCreation from "./EKSRoleCreation";
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {/* <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg rounded-2xl shadow-xl shadow-indigo-500/10 p-6 border border-indigo-100 dark:border-slate-700 mb-6">
            <div>
              ...
              <p className="text-slate-700 dark:text-white">
                Provide the 12-digit AWS Account ID of the account where your
                EKS clusters are deployed. This ensures that access is scoped
                correctly.
              </p>
            </div>
          </div> */}
          ...
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg rounded-2xl shadow-xl shadow-indigo-500/10 p-6 border border-indigo-100 dark:border-slate-700 mb-6">
            <div>
              ...
              <ul className="space-y-3 text-slate-700 dark:text-white">
                ...
                <div>
                  <GetNote
                    note="The stack includes an automation Lambda function that
                  discovers all EKS clusters in all regions and configures
                  access entries with the necessary admin policy for the
                  provided role."
                  />
                </div>
                <li className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <span
                      onClick={async () => {
                        const eks_cft_template_url =
                          process.env.REACT_APP_EKS_CF_TEMPLATE_URL || "";

                        if (eks_cft_template_url) {
                          try {
                            const response = await fetch(eks_cft_template_url, {
                              mode: "cors",
                            });
                            ...
                            link.href = url;
                            link.download = "eks-cft-template.yaml";
                ...
                <li className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <span
                      onClick={async () => {
                        const sopPdfS3Url =
                          process.env.REACT_APP_EKS_SCAN_SOP_PDF_S3_URL || "";
                        if (sopPdfS3Url) {
                          ...
                          const link = document.createElement("a");
                          link.href = url;
                          link.download = "eks-manual-role-creation-steps.pdf";
                          document.body.appendChild(link);
                          link.click();
                          document.body.removeChild(link);
                          window.URL.revokeObjectURL(url);
          ...
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg rounded-2xl shadow-xl shadow-indigo-500/10 p-6 border border-indigo-100 dark:border-slate-700 mb-6">
            <div>
              ...
              <p className="text-slate-700 dark:text-white mb-4">
                Provide the ARN (Amazon Resource Name) of the IAM Role that was
                created in step 1.
                This role enables secure cross-account access so that Security360 can perform security scans on EKS clusters.
              </p>

              <div>
                <GetNote note="This Role is only for EKS scan." />
              </div>
            </div>
          </div>

          {showRoleCreation && (
            <EKSRoleCreation onClose={() => setShowRoleCreation(false)} />
          )}
        </div>
      </div>
    </div>
