...
import PillarDropdown from "../UI/DropDown/PillarDropdown";
// import exportAWAFFindingsToExcel from "./ExportAWAFFindingsToExcel";
...
const AWAFSummary = ({
  ...
  const [AWAFRules, setAWAFRules] = useState(null);
  ...
  const [isAWAFSampleReport, setIsAWAFSampleReport] = useState(false);
  ...
  const [hiddenAWAFFindings, setHiddenAWAFFindings] = useState(() => {
    const saved = localStorage.getItem("hiddenAWAFRules");
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    localStorage.setItem("hiddenAWAFRules", JSON.stringify(hiddenAWAFFindings));
  }, [hiddenAWAFFindings]);

  const handleHideAWAFFinding = (e, findingId) => {
    // console.log("first: ", e, findingId);
    e.stopPropagation();
    setHiddenAWAFFindings((prev) => [...prev, findingId]);
  ...
  const handleReportAccountChange = async (accounts, isSample = false) => {
    ...
    setIsAWAFSampleReport(isSample);
    ...
    try {
      ...
      if (apiResponse?.status === "ok") {
        ...
        const AWAFScannedDataAllRegions = report.results;
        ...
        setAWAFRules(AWAFScannedDataAllRegions);
        processDashboardData(AWAFScannedDataAllRegions);
      } else {
        notifyError(apiResponse?.error_message || "Failed to get report");
        setSelectedReportAccount(previousValue);
        setIsAWAFSampleReport(false);
      }
    } catch (err) {
  ...
  const handleScanClick = async () => {
    ...
    if (!selectedPillar) {
      notifyError("Please select at least one pillar");
      return;
    }

    try {
      // const webSocketURL = `${process.env.REACT_APP_WEBSOCKET_URL}/AWAF-progress`;
      ...
      const sendBackendRequest = async () => {
        setLoading(true);
        try {
          const response = await fetch(`${backend_url}/api/awaf-scan`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          if (result?.status === "ok") {
            ...
            setLoading(false);
            setProgress(0);
            notifyError(result?.error_message || "Error in AWAF rules scan");
            console.log("Error in AWAF rules scan", result);
  ...
  const getFilterOptions = () => {
    ...
    const statuses = [
      ...new Set(
        dashboardData?.tableData.map(
          (item) => statusLabelMap[item.status] || item.status
        )
      ),
    ].filter(Boolean);
  ...
  const getFilteredTableData = () => {
    ...
    return dashboardData?.tableData.filter((item) => {
      ...
      const statusMatch =
        ...
        selectedStatuses
          .map((val) => statusValueMap[val] || val)
      ...
      const hiddenMatch = !hiddenAWAFFindings.includes(item.id + item.region);
  ...
  const exportToJSON = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_AWAF-findings.json`,
      json,
      "application/json"
    );
  ...
  const exportToCSV = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_AWAF-findings.csv`,
      csv,
      "text/csv"
    );
  ...
  const columns = [
    {
      ...
      render: (status) => (
        <div className="flex items-center gap-2">
          <Tag className={getStatusColor(status)}>{formatStatus(status)}</Tag>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Popconfirm
          ...
          onConfirm={(e) => handleHideAWAFFinding(e, record.id + record.region)}
          okText="Yes"
          cancelText="No"
      ...
      <div className="p-6 pl-12">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="mt-2 flex items-center justify-between">
              <h1 className="text-3xl font-bold bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent flex items-center gap-3">
                ...
                AWS Well Architect Framework Review 
              </h1>
            </div>
            ...
            {loading ? (
              ...
              <>
                ...
                <div className="mt-6 flex items-center gap-4">
                  ...
                  >
                    {s3FetchLoading && isAWAFSampleReport ? (
                      <span>
          ...
          {loading ? null : s3FetchLoading ? (
            ...
            <div>
              {isAWAFSampleReport && <GetSampleReportNote />}
              {/* Top Row - Charts */}
                      ...
                      {selectedFinding.fullData ? (
                        <div className="p-6 space-y-8">
                          ...
                          {selectedFinding.fullData.recommendation &&
                            (selectedFinding.fullData.status ==
                              "not_available" ||
                              selectedFinding.affected > 0) && (
                              <>
                                <div>
                                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                          ...
                          {selectedFinding.fullData.aws_doc_link &&
                            (() => {
                              const url = selectedFinding.fullData.aws_doc_link;

                              // Generate a clean readable label based on the URL last segment
                              const label =
                                url
                                  .split("/")
                                  .pop()
