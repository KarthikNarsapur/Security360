...
const CisSummary = ({
  ...
  const [cisRules, setCisRules] = useState(null);
  ...
  const [isCisSampleReport, setIsCisSampleReport] = useState(false);
  ...
  const [hiddenCISFindings, setHiddenCISFindings] = useState(() => {
    const saved = localStorage.getItem("hiddenCISRules");
    return saved ? JSON.parse(saved) : [];
  });

  useEffect(() => {
    localStorage.setItem("hiddenCISRules", JSON.stringify(hiddenCISFindings));
  }, [hiddenCISFindings]);

  const handleHideCISFinding = (e, findingId) => {
    // console.log("first: ", e, findingId);
    e.stopPropagation();
    setHiddenCISFindings((prev) => [...prev, findingId]);
  ...
  const handleReportAccountChange = async (accounts, isSample = false) => {
    ...
    try {
      ...
      if (apiResponse?.status === "ok") {
        ...
        const cisScannedDataAllRegions = report.results;
        ...
        setCisRules(cisScannedDataAllRegions);
        processDashboardData(cisScannedDataAllRegions);
      } else {
        notifyError(apiResponse?.error_message || "Failed to get report");
        setSelectedReportAccount(previousValue);
        setIsCisSampleReport(false);
      }
    } catch (err) {
  ...
  const handleScanClick = async () => {
    ...
    try {
      const webSocketURL = `${process.env.REACT_APP_WEBSOCKET_URL}/cis-progress`;
      ...
      const sendBackendRequest = async () => {
        try {
          const response = await fetch(`${backend_url}/api/cisscan`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
          });
          ...
          if (result?.status === "ok") {
            ...
            setLoading(false);
            setProgress(0);
            notifyError(result?.error_message || "Error in cis rules scan");
            console.log("Error in cis rules scan", result);
  ...
  const getFilteredTableData = () => {
    ...
    return dashboardData?.tableData.filter((item) => {
      ...
      const hiddenMatch = !hiddenCISFindings.includes(item.id + item.region);
  ...
  const exportToJSON = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_cis-findings.json`,
      json,
      "application/json"
    );
  ...
  const exportToCSV = () => {
    ...
    downloadFile(
      `${accountDetails?.[0]?.account_id || ""}_cis-findings.csv`,
      csv,
      "text/csv"
    );
    ...
    },
    {
      ...
      render: (_, record) => (
        <Popconfirm
          ...
          onConfirm={(e) => handleHideCISFinding(e, record.id + record.region)}
          okText="Yes"
          cancelText="No"
  ...
  if (loading) {
    return <LoadingSkeletonCisDashboard progress={progress} />;
  }
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <div className="mt-6 flex items-center gap-4">
              ...
              >
                {s3FetchLoading && isCisSampleReport ? (
                  <span>
          ...
          {s3FetchLoading ? (
            ...
            <div>
              {isCisSampleReport && <GetSampleReportNote />}
              {/* Top Row - Charts */}
