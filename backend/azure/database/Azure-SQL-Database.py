...
class AzureSQLSecurityScanner:
    ...
    # =========================
    def check_sql_public_and_firewall(self, meta):
        findings = []
        total = 0
        ...
        for server in self.sql_client.servers.list():
            ...
            resource_group = self.get_resource_group_from_id(server.id)

            try:
                firewall_rules = self.sql_client.firewall_rules.list_by_server(
        ...
        return {
            "check_name": "Azure SQL Public Access + Firewall",
            "service": "Azure SQL Database",
    ...
    def check_private_endpoint_missing(self, meta):
        ...
        for server in self.sql_client.servers.list():
            ...
            try:
                pe_connections = list(
                    self.sql_client.private_endpoint_connections.list_by_server(
                        resource_group_name=resource_group,
                        server_name=server.name
                    )
                )
    ...
    def check_high_availability(self, meta):
        ...
        for server in self.sql_client.servers.list():
            ...
            if not getattr(server, "fully_qualified_domain_name", None):
                findings.append({
                    "resource_name": server.name
                })
    ...
    def check_entra_auth(self, meta):
        ...
        for server in self.sql_client.servers.list():
            ...
            aad_admin = getattr(server, "administrators", None)
...
scanner = AzureSQLSecurityScanner(subscription_id)
meta = init_scan_metadata()

results = []

results.append(scanner.check_sql_public_and_firewall(meta))
