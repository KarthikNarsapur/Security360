...
class AzureMySQLSecurityScanner:
    ...
    # =========================
    def check_mysql_public_and_network(self, meta):
        findings = []
        ...
        return {
            "check_name": "MySQL Network Exposure",
            "service": "Azure Database for MySQL",
    ...
    def check_delete_locks(self, meta):
        ...
        return {
            "check_name": "MySQL Delete Lock Missing",
            "service": "Azure Database for MySQL",
    ...
    def check_backup_retention(self, meta):
        ...
        for server in self.mysql_client.servers.list():
            ...
            storage_profile = getattr(server, "storage_profile", None)

            if (
                not storage_profile or
                storage_profile.backup_retention_days < 7
            ):
                findings.append({
                    "resource_name": server.name
                })
    ...
    def write_results_to_json(self, results, meta):
        ...
        file_path = script_dir / f"{safe_filename}_mysql.json"
...
scanner = AzureMySQLSecurityScanner(subscription_id)
meta = init_scan_metadata()

results = []

results.append(scanner.check_mysql_public_and_network(meta))
